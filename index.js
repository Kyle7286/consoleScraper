...
const getTarget_PS5 = async (url) => {
    try {
        ...
        // Wait for the element we are interested in to load
        await page.waitForSelector('.h-text-orangeDark');

        // Get the attribute data of the element
        const avail = await page.evaluate('document.querySelector(".h-text-orangeDark").innerHTML')
...
const getAvailability = async () => {
    ...
    report.playstation.target.stan = await getTarget_PS5(url_target_playstation_stan)
    report.playstation.target.dig = await getTarget_PS5(url_target_playstation_dig)
